...
class BS2GRProxy(webapp.RequestHandler):
    ...
    def process(self, allow_cache = True):
        try:
            ...
            old_host = config.old_host
            new_path,_ = config.old_host_exp.subn(config.target_host,self.request.host)
            ...
            if not_processed:
                ...
                if config.abs_url_filter:
                    new_path_qs = path_qs.replace(config.old_host, config.target_host)
                ...
                for _ in range(config.retry):
                    try:
                        resp = urlfetch.fetch(new_path, self.request.body,
                                method, newHeaders, False, False, 30)
                        fetched = True
                        break
                ...
                if config.abs_url_filter:
                    ...
                    if resp.headers.get('Content-Type', '').find('html') >= 0:
                        try:
                            resp.content, tmp = \
                                config.host_exp.subn(config.old_host, resp.content)
                            count += tmp
                        except:
                            logging.info(str(type(resp.content))+
                                    str(type(config.old_host)))
                    if resp.headers.has_key('Location'):
                        resp.headers['Location'], tmp = config.host_exp.subn(config.old_host, resp.headers['Location'])
                        count += tmp
...
""" % (BS2GRPAdmin.BASE_URL, self.request.url))
